...
class StepDialog(tk.Toplevel):
    ...
    def _ok(self) -> None:
        ...
        # OCR fields
        ocr_w = max(0, _int(self._var_ocr_w)) if action == "ocr_read" else 0
        ocr_h = max(0, _int(self._var_ocr_h)) if action == "ocr_read" else 0
        ocr_var = self._var_ocr_var.get().strip() if action == "ocr_read" else ""
        ocr_lang = (self._var_ocr_lang.get() or "eng") if action == "ocr_read" else "eng"
