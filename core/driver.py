...
class WindowsDriver:
    ...
    def find_text_on_screen(
        ...
        """Procura `text` na tela via OCR. Retorna (cx, cy, w, h) ou None.

        Diferente de `find_image_on_screen`, não precisa template — só o texto
        escrito. Funciona com qualquer fonte/cor/tamanho contanto que o OCR
        consiga ler.

        region: (x, y, w, h) — limita a busca a uma região da tela. None = tela
                inteira (mais lento, ~500ms-2s vs ~100-300ms numa região).
        match_mode: "contains" (substring) | "exact" (palavra inteira igual) |
                    "regex" (padrão regex)
        min_confidence: Tesseract retorna -1 a 100 por palavra; descartamos
                        leituras <50% pois costumam ser lixo.

        Retorna o centro da bounding box da MELHOR match (maior confiança),
        ou None se não achou ou pytesseract/Tesseract indisponíveis.
        """
        if not text:
            return None
        try:
        ...
        try:
            if region:
                rx, ry, rw, rh = region
                img = ImageGrab.grab(bbox=(int(rx), int(ry),
                                            int(rx + rw), int(ry + rh)))
                ox, oy = int(rx), int(ry)
            else:
                img = ImageGrab.grab()
                ox, oy = 0, 0
            # image_to_data retorna dict com listas paralelas:
            # text[i], left[i], top[i], width[i], height[i], conf[i]
            data = pytesseract.image_to_data(
                img, lang=lang or "eng",
        ...
        if match_mode == "regex":
            try:
                ...
                pattern = _re.compile(text, flags)
            except Exception:
                return None
        # Itera palavras procurando match com maior confiança
        best: tuple[int, int, int, int, float] | None = None  # x,y,w,h,conf
        n = len(data.get("text", []))
        for i in range(n):
            ...
            if conf < min_confidence:
                continue
            hay = word if case_sensitive else word.lower()
            matched = False
            if match_mode == "exact":
                matched = (hay == needle)
            elif match_mode == "regex":
                matched = bool(pattern.search(word))
            else:  # contains
