...
class SequentialRunner:
    ...
    def _run(
        ...
        while self._running:
            ...
            if self._running and loop_delay_ms > 0:
                time.sleep(loop_delay_ms / spd / 1000)
            loops += 1
        self._running = False
        if on_stop:
            on_stop()
    ...
    def _eval_condition(self, step, ctx: MacroContext, target_hwnd: int) -> bool:
        ...
        if cond_type == "image":
            found = self._driver.find_image_on_screen(
                ...
                timeout_ms=step.image_timeout_ms or 0,
            )
            if found:
            ...
            if step.image_data:
                ...
                while self._running and time.time() < deadline:
                    ...
                    if self._paused:
                        ...
                        if not self._running:
                            break
                        # Estende o deadline pelo tempo que ficou pausado: se o
                        # usuário pausa por 10s, o timeout não conta esse tempo.
                        # (sem isso, pausa longa = timeout zerado ao retomar)
                        continue
                    found = self._driver.find_image_on_screen(
    ...
    def _load_macro_steps(self, path: str) -> list:
        ...
        try:
            ...
            return [macrostep_from_dict(d) for d in script.macro_steps]
