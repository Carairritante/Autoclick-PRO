...
class HotstringManager:
        ...
        with self._lock:
            ...
            for hs in self._hotstrings:
                ...
                if trig and text.endswith(trig):
                    ...
                    force = bool(hs.get("force_type", False))
                    self._buffer.clear()
                    # Schedule fora do hook pra não bloquear o keyboard
                    threading.Thread(
                        target=self._expand,
                        args=(len(trig), expand, force),
                        daemon=True,
                    ).start()
                    if self.on_expand:
    ...
    def _expand(self, trigger_len: int, expand_text: str,
        ...
        with self._expand_lock:
            self._do_expand(trigger_len, expand_text, force_type)

    def _do_expand(self, trigger_len: int, expand_text: str,
                    force_type: bool = False) -> None:
        ...
        if not expand_text:
            return
        # 2a. Modo "forçar digitação": tecla-por-tecla via SendInput Unicode.
        # Necessário pra Roblox e jogos que bloqueiam Ctrl+V no chat.
        # Mais lento mas universal.
        if force_type:
            for ch in expand_text:
                self._driver.perform_type("char", ch)
            return
        # 2b. Modo padrão (paste): instantâneo, funciona na maioria dos apps.
